...
Перехожу в аналитику данных из бизнес- и системного анализа. 4,5 года в RPA-компании: собирала требования, писала ТЗ и координировала запуск решений для банков и ритейла; полгода вела команду из 11 человек как менеджер проекта. Последние 1,5 года углубляюсь в аналитику данных — от A/B-тестов и юнит-экономики до ETL-пайплайнов и дашбордов, собрала портфолио из реальных проектов.
...
**Инструменты**: Python (pandas, scikit-learn), DuckDB, Parquet, звёздная схема, оконные функции SQL, Streamlit, Altair, GitHub Actions (CI); интервал Уилсона с поправкой Бонферрони, Z-тест долей, KMeans, проверка прогноза на отложенных данных  
**Результат**: Живой дашборд на Streamlit — 6 вкладок, 3 источника данных, основной — ~26 000 матчей. В общей таблице ни один чемпион не выделяется с учётом множественных сравнений, а внутри ролей сильные и слабые появляются — вывод зависит от способа подсчёта. Прогноз победителя по чемпионам проверен на матчах последнего патча, которых модель не видела, и уступает простому правилу «побеждает сторона карты»; игроки разделены на стили игры (KMeans)  
...
### 💝 Проект 2: Анализ регулярных пожертвований благотворительного фонда («Марафон добра»)
**Задача**: По выгрузке 42,8 тыс. подписок на ежемесячные пожертвования разобраться, почему фонд теряет регулярных донаторов, кто приносит основные сборы и что поможет их вырастить. Заказчик — фонд «Марафон добра»  
**Инструменты**: Python (pandas, lifelines), когортный анализ, анализ выживаемости (Каплан–Майер, логранк-тест), RFM и ABC/Парето, SQL (DuckDB) для перекрёстной проверки расчётов, Power BI  
**Результат**: Отток на две трети оказался техническим: подписка не доживает до первого списания или не проходит карта. Подписки, оформленные с 2023 года и прошедшие первое списание, живут **в 3,4 раза дольше** прежних, а четверть плативших донаторов даёт **80% сборов**. Главный риск — в январе–мае 2026 года новых подписок на 45% меньше, чем годом раньше; для каждого сегмента донаторов сформулированы рекомендации  
[🔗 Подробнее →](https://github.com/AnastasiaPatrusheva/donor-analytics)

### 🎓 Проект 3: A/B-тест образовательного онбординга в финтехе (SollmaFin)
...
### 📈 Проект 4: Юнит-экономика и оптимизация каналов привлечения (Procrastinate Pro+)
...
### 🛒 Проект 5: Поиск точек роста маркетплейса + микро-эксперимент
...
### ⚙️ Проект 6: ETL-пайплайн аналитики стримингового сервиса
